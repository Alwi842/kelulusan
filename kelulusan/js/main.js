function logout(){
	var result = confirm("Apakah anda yakin mau log out?");
	if (result) {
...
function ppdb_settings(){
	...
	window.location.replace("ppdb_settings.php?setting=1&tahun_ajar="+tahun);
}
function fileName(file){
	if (file==1) {
		...
		filename_kk=document.getElementById("akta").files[0].name;
		document.getElementById("akta_btn").innerHTML=filename_kk;
	}
}
function hapus(kode_pendaftaran){
	var result = confirm("Apakah anda yakin ingin menghapus?");
	if (result) {
		window.location.replace("control.php?action=2&kode_pendaftaran="+kode_pendaftaran);
	} else return false;
}
...
function show_list() {
	...
	window.location.replace("list.php?page=1&showtotal="+nilai);
}
